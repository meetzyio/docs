...
This filter can be shown or hidden from the filter visibility panel described above. When enabled, it appears in the Filter Bar alongside the other filter controls.
...
<Tip>
Like the Tags and Evaluations filters, the Call ID field itself can be hidden from the Filter Bar using the filter visibility settings if your workflow doesn't rely on manual ID lookups.
</Tip>
...
This filter can be toggled on or off from the filter visibility settings panel, and — like all other filters — is temporarily disabled while a Call ID search is active.
...
This filter is hidden by default in a minimal setup and can be enabled from the filter visibility panel whenever you need deeper evaluation-based analysis. It is also disabled while a Call ID search is active, following the same priority behavior as the rest of the filters.
...
| ⚙️ Settings Icon | Opens the filter visibility configuration panel |
...
<CardGroup cols={2}>
  ...
  <Card title="Use Call ID for Debugging" icon="hashtag">
    When investigating specific issues, use the exact call ID for instant lookup with database search. Remember it temporarily overrides all other filters.
  </Card>
  <Card title="Customize Filter Visibility" icon="eye">
    Use the ⚙️ settings button to hide Goal, Tags, Evaluations, or Call ID filters you don't use regularly, keeping the Filter Bar focused on what matters to you.
  </Card>
...
Use dynamic tags to identify calls that need quality review. Enable the Tags filter from the settings panel if it isn't already visible.
</Step>
