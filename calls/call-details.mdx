...
<CardGroup cols={2}>
  <Card title="Previous / Next buttons" icon="arrow-right-arrow-left">
    Two chevron buttons sit in the modal header, right next to the close button. Click **‹** to load the previous call's recording and transcript, or **›** to load the next one, all without leaving the modal.
  </Card>
...
Switching calls triggers a short slide transition (the content pane animates left or right depending on the direction you navigated) so it's clear a new recording has loaded, and reloads the waveform, audio, and transcript for the newly selected call.
...
<CardGroup cols={2}>
  <Card title="OK / KO control" icon="thumbs-up">
    A two-option toggle in the modal lets you mark the call currently loaded as **OK** (call handled correctly) or **KO** (call needs attention), with the active option visually highlighted.
  </Card>
  <Card title="Stays with the call" icon="link">
    The verdict is saved against the call you're viewing and travels with it — navigating to the previous/next call loads that call's own verdict state, it never carries over from the call you just left.
  </Card>
</CardGroup>

<Steps>
  <Step title="Open a recording">
    Open any call's recording from Call History or another filtered list.
  </Step>
  ...
  <Step title="Set the verdict">
    Click **OK** or **KO** in the modal to record your judgment for this call.
  </Step>
...
<Info>
Combining the OK/KO control with previous/next navigation and the arrow-key shortcuts turns batch call review into a fast, keyboard-friendly workflow: listen, judge, move on.
</Info>

<Warning>
Setting a verdict updates the call's review status immediately. If you need to also leave a written explanation, use the review note field alongside the OK/KO control — the two are independent and both persist with the call.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Quality Assurance" icon="check-double">
    Listen to recordings, check transcriptions, and mark each call OK/KO to build a fast, keyboard-driven QA workflow.
  </Card>
...
<Info>
There is no dedicated shortcut for the OK/KO verdict — use the on-screen control in the modal. This keeps the verdict a deliberate click rather than something that could be triggered accidentally while navigating between calls.
</Info>
