...
<CardGroup cols={2}>
  <Card title="Previous / Next buttons" icon="arrow-right-arrow-left">
    Two chevron buttons sit in the modal header, next to the close button. Click them to load the adjacent call's recording and transcript in place.
  </Card>
  <Card title="Keyboard shortcuts" icon="keyboard">
    Press **←** to go to the previous call and **→** to go to the next call, without touching the mouse.
  </Card>
</CardGroup>

<Info>
The previous/next buttons are automatically disabled at either end of the list — for example, if you're already viewing the first call, the "previous" button (and the ← shortcut) has no effect.
</Info>
...
Switching calls triggers a short slide transition so it's clear a new recording has loaded, and reloads the waveform, audio, and transcript for the newly selected call.
