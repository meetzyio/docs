...
<Info>
The recording player described below (waveform, navigation, review, and tags) is a single shared component. It now also powers the recording modal in the **advanced Calls view**, so whichever screen you review calls from, the behavior and shortcuts documented here are identical.
</Info>
...
<Info>
This is the same recording modal used across Meetzy — including the **advanced Calls view** — so the waveform player, keyboard navigation, and review/tag editing described below all work identically no matter which list you opened it from.
</Info>

### Navigating Between Calls

The recording modal no longer traps you on a single call. When you open a recording from a filtered list (for example, from Call History), the modal knows about the calls immediately before and after the one you're viewing, and lets you move between them without closing the modal and reopening a different row.

<CardGroup cols={2}>
  <Card title="Previous / Next buttons" icon="arrow-right-arrow-left">
...
Switching calls triggers a short slide transition (the content pane animates left or right depending on the direction you navigated) so it's clear a new recording has loaded, and reloads the waveform, audio, transcript, review status, and tags for the newly selected call.

### Call ID Badge Next to the Transcript

To make it easy to reference a specific call while reviewing recordings — for example, when filing a bug report or pasting the ID into a support ticket — the **Call ID** is now shown as a small badge right above the transcript panel, next to the "Transcript" heading.

<CardGroup cols={2}>
  <Card title="Always visible" icon="id-badge">
...
Reviewing a batch of recordings usually ends with the same decision for each call: was this a good call or not? Instead of closing the modal and marking the call from the table, you can now record that verdict right from the recording player.

The verdict control is **tri-state**, not a simple on/off toggle:

| State | Meaning |
|-------|---------|
| Pending | Default state — the call has not been reviewed yet, or the verdict was cleared |
| OK | The call was handled correctly by the agent |
| KO | The call needs attention (bad handling, missed data, wrong tone, etc.) |

<CardGroup cols={2}>
  <Card title="OK / KO control" icon="thumbs-up">
  ...
  <Card title="Click again to deselect" icon="rotate-left">
...
<Steps>
  ...
  <Step title="Move to the next call">
...
<Warning>
Setting a verdict updates the call's review status immediately. If you need to also leave a written explanation, use the review note field alongside the verdict control — the two are independent and both persist with the call.
</Warning>

### Reviewed Status, Notes, and Tags — Right From the Modal

Full review editing now lives inside the recording modal itself, so you don't need to go back to the table row to finish reviewing a call.

<CardGroup cols={2}>
  <Card title="Reviewed checkbox" icon="check-double">
  ...
  <Card title="Review note" icon="pen">
  ...
  <Card title="Follows the call" icon="arrow-right-arrow-left">
...
Next to the player, the transcript is rendered turn-by-turn (not as a single wall of text), with the [Call ID badge](#call-id-badge-next-to-the-transcript) sitting right next to the "Transcript" heading. Each turn shows its elapsed time in the call, and agent turns carry two kinds of expandable detail sourced directly from the underlying ElevenLabs conversation: a **response time breakdown** and, when applicable, **tool call details**.
...
Each stage in the breakdown shows its duration and percentage of the total, along with the LLM model that generated the response for that turn. This turns a single opaque latency number into an actionable diagnosis:

- A response dominated by **Understanding speech** may point to background noise or a caller who talks quickly.
- A response dominated by **Model thinking** may point to a slow or overloaded LLM — worth checking against the [LLM Model Usage](#llm-model-usage) section above to see if a cascade fallback happened on that same turn.
...
<Steps>
  ...
  <Step title="Check for deliberate waits">
...
To make tool activity scannable at a glance, the transcript now uses a consistent color code for anything related to tools — both in the badges next to a turn and in the expanded detail cards below it:

| Color | Meaning |
|-------|---------|
| 🟠 Amber | A tool call — the request the agent sent to a tool, with the tool name and parameters |
| 🔵 Blue | A tool response — the result the tool returned for that call |
| 🔴 Red | A tool error — the call failed, and the card shows the error details instead of a result |

<Info>
This color coding is applied consistently across the modal: the small **🔧 tool name** badge next to a turn, and the expanded parameters/result card underneath it, both use the same amber/blue/red scheme. A quick glance down the transcript is enough to spot which turns involved tools and whether any of those calls failed.
</Info>
...
<Warning>
If the underlying ElevenLabs conversation is no longer available (for example, an old or purged call), the modal automatically falls back to a **plain transcript** view — the message bubbles are still shown, but without latency breakdowns, model detail, or expandable tool call/result cards. A short notice appears above the transcript in this case. The same amber/blue/red color coding for tool calls, responses, and errors is preserved in this fallback mode wherever that information is still present in the raw transcript, so tool activity remains easy to spot even without the rich per-turn metrics.
</Warning>
...
| `←` | In the recording modal: jump to the previous call |
| `→` | In the recording modal: jump to the next call |
