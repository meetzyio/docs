...
Unlike a plain `<audio>` progress bar, the player renders a **real waveform** generated directly from the decoded audio — not a placeholder animation. When the recording loads, the audio is decoded via the Web Audio API and sampled into 100 amplitude bars, so the shape you see actually reflects the call's audio (silences, loud segments, etc.).
...
Next to the player, the transcript is rendered turn-by-turn (not as a single wall of text), and each turn can carry extra detail sourced directly from the underlying ElevenLabs conversation:

| Badge | Shown on | Meaning |
|-------|----------|---------|
| ASR | User turns | Speech-to-text latency for that turn |
| TTS | Agent turns | Text-to-speech latency for that turn |
| LLM | Agent turns | Time-to-first-byte from the language model, expandable to show the exact model used and detailed LLM metrics |
| 🔧 Tool name | Agent turns | A tool/function call made during that turn, expandable to show its parameters and result |

<Steps>
  <Step title="Scan the badges">
    Each message bubble shows its elapsed time in the call, plus any latency badges relevant to that turn.
  </Step>
  <Step title="Expand LLM detail">
    Click the **LLM** badge to reveal which model generated that response and a breakdown of LLM-related metrics (time to first byte, time to first/last sentence, etc.).
  </Step>
  <Step title="Expand tool calls">
    Click a **🔧 tool name** badge to reveal the parameters sent to the tool and the result it returned (or the error, if the call failed).
  </Step>
</Steps>
...
<Warning>
If the underlying ElevenLabs conversation is no longer available (for example, an old or purged call), the modal automatically falls back to a **plain transcript** view — the message bubbles are still shown, but without latency badges, model detail, or tool call/result cards. A short notice appears above the transcript in this case.
</Warning>
