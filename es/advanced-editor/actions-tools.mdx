...
<Tabs>
  <Tab title="Español - Transferencia">
    ```text
  ...
  <Tab title="Español - Finalización de Llamada">
    ```text
    ## Protocolo de Finalización de Llamada
    
    DETECCIÓN DE CONTESTADOR AUTOMÁTICO:
    Finalizar inmediatamente cuando se detecten estas frases:
    - "Deje su mensaje después de la señal"
    - "En este momento no podemos atenderle"
    - "El número marcado no está disponible"
    - "Gracias por llamar a [empresa/servicio]"
    - "Buzón de voz de [nombre/empresa]"
    - "Ha contactado con el servicio de..."
    - "Bienvenido al contestador automático"
    - "Su llamada es importante para nosotros"
    - "No podemos atender su llamada ahora"
    - "Por favor, inténtelo de nuevo más tarde"
    
    FINALIZACIÓN POR CUMPLIMIENTO DE OBJETIVOS:
    - Todos los objetivos de la llamada han sido completados
    - El cliente ha proporcionado toda la información requerida
    - Se ha completado exitosamente el proceso solicitado
    - El cliente confirma satisfacción con la resolución
    
    FINALIZACIÓN POR DESPEDIDA NATURAL:
    - El cliente se despide de forma clara e inequívoca
    - Intercambio de despedidas protocolarias completado
    - El cliente indica que no necesita nada más
    - Finalización cortés y profesional del diálogo
    ```
  </Tab>
  <Tab title="Español - Protocolo Recall">
    ```text
    ## Protocolo de Rellamada Automática
    
    ACTIVAR RELLAMADA EN ESTOS CASOS:
    
    CONDICIONES TÉCNICAS:
    - No se obtiene respuesta después de timbre prolongado
    - La línea telefónica está ocupada o congestionada
    - Se detecta contestador automático o buzón de voz
    - La llamada se corta inesperadamente por fallas técnicas
    - Problemas de calidad de audio que impiden conversación
    - Rechazo automático por parte del sistema del operador
    
    CONDICIONES DE NEGOCIO:
    - No se completaron los objetivos principales de la llamada
    - El cliente solicitó ser contactado en horario específico
    - Información crítica pendiente de recopilación o confirmación
    - Proceso de venta o gestión interrumpido antes de finalizar
    - Seguimiento requerido según protocolo empresarial
    - Confirmación de cita, servicio, o compromiso pendiente
    
    CONFIGURACIÓN DE REINTENTOS:
    - Máximo 3 intentos de rellamada por contacto
    - Intervalos progresivos: 30 min → 4 horas → 1 día
    - Respetar horarios comerciales (9:00-18:00) zona local
    - Excluir fines de semana y días festivos nacionales
    - Detener rellamadas si cliente solicita no ser contactado
