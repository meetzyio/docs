...
<Info>
Las métricas de latencia de respuesta de la IA (media y máxima, en milisegundos) están disponibles como columna en la tabla de llamadas y también como filtro, permitiéndote identificar rápidamente llamadas con tiempos de respuesta elevados. Además, puedes visualizar la evolución de la latencia turno a turno mediante un mini-gráfico en la propia fila y un gráfico detallado en el modal de la llamada.
</Info>
...
- **Latencia de respuesta de la IA** (media y máxima, en ms), disponible como columna y como filtro
...
### Filtros de Latencia de Respuesta de la IA

Filtra las llamadas por la latencia (en milisegundos) que tardó la IA en responder:
...
<Info>
Este filtro utiliza la latencia media de respuesta de la IA por llamada (`avg_response_latency_ms`). También puedes añadir la latencia máxima (`max_response_latency_ms`) como columna adicional en la tabla para detectar picos puntuales dentro de una misma conversación.
</Info>
...
### Métricas de Latencia de la IA

- **Latencia media de respuesta**: Tiempo promedio (en ms) que tarda la IA en responder tras el silencio del usuario, disponible como columna configurable
- **Latencia máxima de respuesta**: Pico de latencia (en ms) registrado durante la llamada
- **Latencia por turno**: Visualización turno a turno mediante un sparkline en la tabla y un gráfico detallado en el modal de la llamada, útil para detectar degradaciones puntuales de rendimiento

<Info>
Estas métricas de latencia están disponibles tanto como columna visible en la tabla de llamadas como filtro de búsqueda, lo que facilita auditar la calidad de la experiencia conversacional y detectar agentes o momentos con tiempos de respuesta anómalos.
</Info>
...
- **Latencia media de respuesta de la IA**: valores más bajos indican una experiencia conversacional más fluida; latencias elevadas de forma sostenida suelen señalar problemas de configuración o de proveedor de voz/modelo
...
- **Latencia de la IA elevada**: Filtra por latencia mínima/máxima y revisa el gráfico de latencia por turno para identificar en qué momento de la llamada se producen los retrasos
