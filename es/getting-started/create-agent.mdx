...
Al hacer clic en **"Create Agent"** o acceder al Editor Avanzado sin un agente existente, Meetzy te recibe con una pantalla de creación a pantalla completa (`creation-overlay`), pensada para que empieces describiendo tu idea en lugar de rellenar formularios:

- **Fondo Animado WebGL**: Un `<canvas>` de referencia `waveCanvas` renderiza ondas fluidas animadas mediante `requestAnimationFrame`, dando vida a toda la superposición desde el momento en que aparece
...
- **Campo de Texto con Borde Degradado**: Un `textarea` central de 4 filas, envuelto en un contenedor con borde en gradiente (`creation-overlay__border-wrap`) que se adapta automáticamente a modo claro (`--light`) y oscuro (`--dark`)
...
- **Logo Centrado**: El logo de Meetzy se posiciona en la parte superior central y cambia automáticamente entre su versión clara y oscura (`meetzy_logo_transparent.png` / `meetzy_white.png`) según el tema activo
...
- **Botón de Envío Dinámico**: Un botón circular verde con icono de avión de papel aparece en la esquina inferior derecha del campo de texto únicamente cuando `creationPrompt.trim()` detecta contenido escrito
- **Hint de Atajo**: Debajo del campo, un indicador con la tecla `Enter` recuerda cómo generar la configuración, junto al enlace **"or start from scratch"** para empezar en blanco
...
<Steps>
  ...
  <Step title="Genera la configuración">
    En cuanto detecta texto en el campo, aparece el botón de envío en la esquina inferior derecha. Puedes:
    - Presionar **Enter** para lanzar la generación (`onCreationPromptKeydown`)
    - Hacer clic directamente en el botón con el icono de avión de papel (`launchFromCreationOverlay()`)
  ...
  <Step title="Continúa en el Editor Avanzado">
    Tras la generación, la pantalla de creación se cierra con una animación de salida (clase `creation-overlay--exiting`) y aterrizas directamente en el Editor Avanzado con tu agente ya configurado, listo para refinar cualquier detalle.
  </Step>
</Steps>
...
| Enviar con el ratón | Haz clic en el botón de envío (avión de papel) una vez que hay texto |
...
<Info>
  Además del modo de destino único, Actions & Tools ofrece un modo **beta de transferencia con múltiples reglas** (`transferConfigVersion: 'v2'`): en lugar de un solo número de destino, puedes definir varias condiciones (por ejemplo, "si el cliente pide hablar con ventas") cada una con su propio destino, ya sea un número de teléfono o una extensión SIP.
</Info>
...
<Tabs>
  ...
  <Tab title="Cliente Difícil">
    ```
    - Tono frustrado o impaciente
    - Historial de problemas previos
    - Solicita hablar con un humano
    - Cuestiona la legitimidad del agente
    - Requiere manejo empático de la situación
    ```
  </Tab>
</Tabs>

<Info>
  Usa el Playground con frecuencia mientras ajustas el prompt de sistema: te permite validar cambios en segundos, sin necesidad de realizar una llamada real de prueba.
</Info>