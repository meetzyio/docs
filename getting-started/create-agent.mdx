...
<Tabs>
  <Tab title="Prompt-First Creation (Recommended)">
    The fastest way to get started. Simply describe what your agent should do, and Meetzy builds it for you automatically.
  </Tab>
  <Tab title="Advanced Editor">
    Full control over every aspect of your agent's configuration using our three-panel interface.
  </Tab>
</Tabs>

## Prompt-First Creation

When creating a new agent, Meetzy presents an intuitive creation overlay — a full-screen screen with an animated wave background — where you can describe your agent in natural language and let **Meetzy Copilot** generate the initial configuration for you.
...
As soon as you click "New Agent", instead of dropping you into an empty editor, Meetzy shows a dedicated creation screen:

- A centered heading, **"What should your agent do?"**, with a short subtitle inviting you to describe it.
- A large textarea with a soft gradient border where you write your description.
- A send button (arrow icon) that appears as soon as you start typing.
- A subtle animated wave background that automatically adapts to your light/dark theme.
- The Meetzy logo at the top center and a back button at the top left.
...
<Steps>
  <Step title="Describe Your Agent">
    ...
    ```
    A sales qualification bot for a real estate company that qualifies 
    leads, schedules property viewings, and collects contact information.
    ```
    
    ```
    Customer service agent for an e-commerce store that handles order 
    inquiries, processes returns, and provides shipping updates.
    ```
  </Step>
  
  <Step title="Generate Your Agent">
    Press **Enter** or click the send button (arrow icon) to launch Meetzy Copilot. It will automatically:
...
<Info>
  Click **"or start from scratch"** below the textarea (or press **Escape**) to dismiss the creation screen instantly and land directly in the Advanced Editor with a blank configuration.
</Info>

This shortcut is ideal for advanced users who already know exactly what they want to configure, or when you're duplicating patterns you've used before.

### Creation Screen Features

- **Animated Background**: A beautiful WebGL wave animation that adapts to your theme
- **Keyboard Shortcuts**:
...
<Info>
  If you came from our onboarding process, your agent configuration has been automatically preserved and you'll be redirected directly to the agent editor with your initial settings applied — the prompt-first creation screen is skipped in this case.
</Info>

### How It Works

<Steps>
  <Step title="Onboarding Data Preservation">
  ...
  <Step title="Pre-populated Configuration">
