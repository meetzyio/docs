...
## Business Hours Tool (Beta)

<Info>
**Business Hours** is a new built-in tool (Beta) that lets your agent automatically resolve whether the current call is happening within your configured business hours — no webhook or external calendar needed.
</Info>

Instead of building custom logic or prompting the model to reason about dates and time zones, the Business Hours tool evaluates the call timestamp against your schedule and returns a simple `open` / `closed` result the agent can act on.

### What It Solves

- Avoids transferring calls to a human team that isn't available
- Lets the agent give accurate opening hours or "we're currently closed" messaging
- Enables conditional logic such as "only offer callback scheduling outside business hours"
...
<Steps>
  ...
  <Step title="Set Your Schedule">
    Define opening and closing times for each day of the week. Mark days as fully closed when applicable (e.g., weekends or holidays).
  </Step>
...
```
Rule 1
Condition: the customer wants to talk to sales AND the call is within business hours
Destination: Phone → +1-555-200-1000
Type: Warm

Rule 2
Condition: the customer wants to talk to sales AND the call is outside business hours
Destination: (no transfer — offer to schedule a callback instead)
```

**Combined with End Call Strategy or system prompt:**

Instruct the agent to check the Business Hours result before offering live transfer, and fall back to taking a message or booking a callback when the result is `closed`.

<Warning>
Business Hours is currently in **Beta**. Double-check the resolved status during testing, especially around daylight saving time transitions and holiday exceptions, which may require manual configuration.
</Warning>
...
<Info>
Pair the Recall Protocol with the **Business Hours Tool (Beta)** to automatically restrict callback attempts to your configured working hours instead of manually setting time windows.
</Info>
