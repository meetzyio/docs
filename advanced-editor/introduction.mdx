...
| 📞 Recent Calls | Browse the agent's most recent calls without leaving the editor | — |

<Info>
The **Recent Calls** tab shows the last 30 days of calls for the agent you're currently editing, including status, success outcome, duration, and a quick summary. Click any call to open its full recording and transcript in a new tab, or use "View all" to jump to the Calls page filtered for this agent.
</Info>

## Best Practices Guide

Building a great agent isn't only about wiring up settings correctly, it's also about writing clear prompts, structuring greetings well, and choosing the right skills for the job. To help with that, the Advanced Editor now includes a built-in **Best Practices Guide** that you can open at any time, right from the toolbar, without ever leaving your current agent.

<Info>
The Best Practices Guide is contextual documentation designed to help you build better agents faster. It's especially useful right after using the Creation Overlay, when you're fine-tuning a Foundation-generated setup for the first time.
</Info>

### How to Open the Guide

<Steps>
  <Step title="Locate the Guide Icon">
    Look for the **book icon** ("Best Practices") in the Advanced Editor's top toolbar, next to the other editor actions.
  </Step>
  <Step title="Click to Open">
    Click the icon to open the Best Practices modal. A short loading state appears briefly while the guide content is fetched.
  </Step>
  <Step title="Read the Guide">
    Browse the guide's sections directly inside the modal, no need to leave the editor or lose your current work.
  </Step>
  <Step title="Close and Keep Editing">
    Close the modal at any time to return to exactly where you left off in your configuration.
  </Step>
</Steps>

<Warning>
Opening the Best Practices Guide never affects your unsaved changes, Git status, or auto-save behavior. It's a read-only overlay on top of your current editing session.
</Warning>

### What the Guide Covers

The Best Practices Guide is organized to walk you through the most important decisions when building an agent:

- **Writing effective system prompts**: how to structure instructions clearly, avoid ambiguity, and keep prompts maintainable as they grow
- **Crafting greetings that convert**: tips for inbound and outbound openers that set the right tone from the first second of the call
- **Choosing and configuring skills**: guidance on when to use built-in skills versus custom logic, and how to avoid over-configuring an agent
- **Getting the most out of Foundation components**: recommendations for adapting industry templates to your specific business instead of using them as-is
- **Testing before you publish**: a checklist-style overview of what to validate in Playground and Tests before deploying to production
- **Common pitfalls to avoid**: real-world mistakes that lead to unreliable agents, along with practical fixes

<Info>
Think of the Best Practices Guide as a quick reference you can consult while you work, rather than a full manual you need to read end-to-end before getting started.
</Info>

## Focus Mode - Distraction-Free Editing

Sometimes you just need to concentrate on the **System Prompt** without the visual noise of configuration panels and testing tools around you. **Focus Mode** does exactly that: it hides the left configuration panel and the right panel (Meetzy Copilot, Playground, Tests, Changes, Recent Calls), leaving only the Monaco editor centered on your screen.

<Info>
Toggle Focus Mode instantly with the keyboard shortcut **⌘\\** on macOS or **Ctrl+\\** on Windows/Linux. Press it again to bring the side panels back.
</Info>

### How Focus Mode Works

<Steps>
  <Step title="Trigger Focus Mode">
    Press **Cmd+\\** (macOS) or **Ctrl+\\** (Windows/Linux) anywhere in the Advanced Editor.
  </Step>
  <Step title="Panels Disappear">
    The left configuration panel and the right panel (Copilot, Playground, Tests, Changes, Recent Calls) smoothly collapse out of view.
  </Step>
  <Step title="Editor Takes Center Stage">
    The Monaco editor expands to use the freed-up space, giving you a clean, full-width writing surface for your system prompt.
  </Step>
  <Step title="Exit Anytime">
    Press the same shortcut again to instantly restore the panels and return to the standard layout.
  </Step>
</Steps>

### When to Use Focus Mode

- **Writing or rewriting long system prompts** where every distraction counts
- **Reviewing prompt structure** using the outline without side panel clutter
- **Screen-sharing or recording demos** where a clean interface looks more professional
- **Working on smaller screens** where side panels compete for space with the editor

<Warning>
Focus Mode only hides the panels visually, it does not disable your unsaved changes protection, Git status tracking, or auto-save behavior. All your work is preserved exactly as before.
</Warning>

#### Expand the Right Panel for a Better Reading Experience

The right panel now includes an **expand button** that widens the panel so you can read Meetzy Copilot's messages, Playground results, Tests, Changes history, and Recent Calls much more comfortably.
...
It's a small addition, but once you toggle it on, you'll immediately notice how much easier it is to follow long Copilot explanations, compare Playground conversation turns, review detailed diffs in the Changes tab, or scan through Recent Calls, no more squinting at cramped text in a narrow column.

<Steps>
  <Step title="Open any right panel tab">
    Select Meetzy, Playground, Tests, Changes, or Recent Calls from the right panel tabs.
  </Step>
  ...
  <Step title="Read comfortably">
    Enjoy a wider, more readable layout for long messages, test results, change diffs, or call summaries.
  </Step>
...
<Info>
Focus Mode and the expandable right panel work great together: use Focus Mode when you need to write without distractions, and expand the right panel when you need to read long Copilot answers, Playground transcripts, Changes diffs, or Recent Calls details.
</Info>
...
<Steps>
  ...
  <Step title="Open the Best Practices Guide">
    If it's your first time fine-tuning an agent, open the Best Practices Guide from the toolbar to review recommendations on prompts, greetings, and skills before making changes.
  </Step>
  ...
  <Step title="Fine-tune Foundation Configuration">
    Adjust Foundation-powered settings, greetings, and system prompts generated from your initial description. Toggle Focus Mode (⌘\\ / Ctrl+\\) if you want to write without distractions.
  </Step>
  ...
  <Step title="Check Recent Calls">
    Once your agent is live, use the Recent Calls tab to quickly review real conversations without leaving the editor, spotting issues or opportunities to refine your prompt.
  </Step>
...
### A Guide That Meets You Where You Are

The Best Practices Guide was built with one goal in mind: help you build better agents without breaking your flow.

- **One click away**: always accessible from the toolbar, no matter which section you're editing
- **Non-intrusive**: opens as a modal overlay, your configuration state is never touched
- **Focused content**: organized around the real decisions you make while building an agent, not generic marketing copy
- **Great for onboarding**: a natural next step after the Creation Overlay, right when you start customizing a Foundation-generated agent

<Info>
Consider making the Best Practices Guide your first stop whenever you're unsure whether a prompt, greeting, or skill configuration follows recommended patterns.
</Info>

### Focus Mode for Deep Work

Focus Mode is designed for the moments when you need to think clearly about your agent's behavior:

- **One shortcut, instant results**: ⌘\\ (macOS) or Ctrl+\\ (Windows/Linux) toggles the layout immediately
- **Zero configuration lost**: hiding panels never affects your unsaved changes, Git status, or auto-save
- **Full-width editing**: the Monaco editor expands to use all the available space
- **Toggle anytime**: press the shortcut again to bring back the left and right panels instantly

<Info>
Focus Mode is a purely visual toggle, think of it as closing the door to your office. Everything you were working on is still there when you open it again.
</Info>
...
- **More scannable calls** when browsing Recent Calls entries and summaries
...
### Recent Calls, Right Where You Need It

Reviewing real conversations used to mean leaving the editor entirely and navigating to a separate Calls page. Now, the **Recent Calls** tab brings the last 30 days of activity for the agent you're editing directly into the right panel:

- **At-a-glance status**: color-coded badges for finished, failed, busy, ringing, and other call outcomes
- **Success indicators**: quickly spot calls marked as successful based on your evaluation criteria
- **Relative and absolute timestamps**: see "2 hours ago" at a glance, with the exact date and time available on hover
- **One-click access**: open any call's full recording and transcript in a new tab without losing your place in the editor
- **View all**: jump straight to the Calls page pre-filtered for the current agent when you need the complete history

<Info>
Recent Calls refreshes on demand with a single click, so you can always check the very latest interactions while you're mid-edit on your system prompt.
</Info>
