...
The Advanced Editor now includes an **integrated agent selector** in the toolbar that allows you to seamlessly switch between different agents without leaving the editor. This powerful productivity feature saves valuable time by eliminating the need to navigate back to the agents dashboard.

<Info>
The agent selector displays the current agent name as a dropdown button, making it easy to identify which agent you're working on and quickly switch to another one.
</Info>

#### Agent Selector Features

- **Quick Agent Switching**: Click the agent name to reveal a dropdown with all available agents
- **Search Functionality**: Find agents quickly using the built-in search box
- **Visual Identification**: Each agent shows an avatar with the first letter of its name
- **Current Agent Highlighting**: The currently selected agent is clearly marked with a green indicator
- **Agent ID Display**: Shortened agent IDs help distinguish between similarly named agents

#### Using the Agent Selector

<Steps>
  <Step title="Access the Selector">
    Click on the current agent name in the toolbar (shows a dropdown arrow when available)
  </Step>
  <Step title="Search for Agents">
    Use the search box at the top of the dropdown to filter agents by name
  </Step>
  <Step title="Select an Agent">
    Click on any agent in the list to switch to it immediately
  </Step>
  <Step title="Verify Selection">
    The toolbar updates to show the new agent name and the dropdown closes automatically
  </Step>
</Steps>

<Warning>
If you have unsaved changes when switching agents, you'll be prompted to save or discard them before proceeding.
</Warning>
