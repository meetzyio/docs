---
title: 'Copilot'
description: 'AI-powered system engineer that transforms your agent with intelligent prompt editing, parameter configuration, rich visual diagnostics, conversation management, file attachments, and voice dictation'
icon: 'robot'
---
...
Enhanced natural interaction capabilities for comprehensive system configuration - including hands-free dictation for writing your messages.
  ...
  <Tab title="Voice Dictation">
    ...
    <Steps>
      <Step title="Start Recording">
        ...
        - **Hold the microphone button** next to the input field
        - **Double-press Shift** to start hands-free recording

        While recording, the input area shows a **live waveform visualizer** and a pulsing red dot so you always know you're being recorded.
      </Step>
      <Step title="Stop and Transcribe">
        Click the microphone button again (now shown as a stop icon) to end the recording. Copilot automatically:
    ...
    <Info>
    Short audio cues play when recording starts and stops, so you get clear feedback even without looking at the screen. If transcription fails or no audio was captured, the input simply stays empty - nothing is lost, and you can try again.
    </Info>
  </Tab>
...
```
Double-press Shift, say: "Update the greeting to sound warmer and 
mention our new extended support hours", then press the mic button to stop.
```
...
<Info>
Copilot maintains conversation context across multiple system modification requests, allowing for iterative refinement of your entire agent configuration through natural dialogue. Use the **thread selector** to keep unrelated iteration threads separate - for example, running your lead-qualification setup in one conversation while diagnosing an unrelated issue in a second thread, then switching between them without losing either context.
</Info>
...
| **Shift + Shift** | Start/stop voice recording (dictation) |
| **⌘Enter** | Send message to Copilot |
| **Enter** | Send message (Shift+Enter for a new line) |
| **Escape** | Close suggestions or cancel an in-progress recording |
...
<Steps>
  ...
  <Step title="Leverage Real Call Data for System Design">
    **Reference actual calls to drive system-wide improvements**:
    ...
    - Use `@call:ID` syntax to provide specific examples of system g