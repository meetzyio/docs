...
<Steps>
  ...
  <Step title="Review All Affected Configuration Areas">
    **Check every panel with purple notification indicators**:
    ...
    - Output Settings for enhanced data capture **and any proposed Output Parameter or Output Enrichment changes**
  ...
  <Step title="Review Output Changes Before Accepting Them">
    **Treat Output Parameter and Enrichment suggestions like any other change**:
    
    - Open the Output Settings panel whenever Copilot proposes output-related changes
    - Read each pending card's operation (Add/Modify/Remove) and field details before deciding
    - Use individual Accept/Reject for surgical control, or bulk actions when you trust the full batch
    - Nothing is applied to your agent until you explicitly accept it
  </Step>
...
<CardGroup cols={2}>
  ...
  >
    Set up output fields, enrichment, and webhooks with intelligent recommendations - now including Copilot's own accept/reject pending-changes panel
  </Card>
  <Card
    title="Evaluations"
    icon="chart-bar"
